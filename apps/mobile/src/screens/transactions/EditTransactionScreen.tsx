...
// Converte ISO da TransacaoData para Date local. T12:00:00 evita
// que fuso horário inverta o dia ao parsear.
...
export default function EditTransactionScreen({ route, navigation }: Props) {
  ...
  const [valorRaw, setValorRaw] = useState(formatValorInput(String(Math.round(transacao.valor * 100))));
  const [descricao, setDescricao] = useState(transacao.descricao);
  const [dataSelecionada, setDataSelecionada] = useState(parseISOToDate(transacao.data));
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [tempDate, setTempDate] = useState(parseISOToDate(transacao.data));
  const [categoriaSelecionada, setCategoriaSelecionada] = useState(transacao.categoria_nome ?? '');
  ...
  async function handleSalvar() {
    ...
    setLoading(true);
    try {
      await api.transacoes.atualizar(transacao.id, {
    ...
    >
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'left', 'right']}>
        ...
        >
          {/* Tipo (read-only) */}
          <View
            accessibilityLabel={`Tipo: ${isDespesa ? 'Despesa' : 'Receita'} (não editável)`}
            style={{
          ...
          >
            <View style={{
              ...
              <Ionicons
                name={isDespesa ? 'trending-down' : 'trending-up'}
                size={16}
                color={T.sub}
              />
              <Text style={{ fontSize: 14, fontWeight: '700', letterSpacing: 0.2, color: T.sub }}>
                {isDespesa ? 'Despesa' : 'Receita'}
              </Text>
              <Ionicons name="lock-closed" size={11} color={T.muted} style={{ marginLeft: 4 }} />
            </View>
          </View>
        ...
        <View style={{
          ...
          >
            {loading ? (
              ...
              <>
                <Ionicons name="checkmark-circle-outline" size={20} color="#09091A" />
